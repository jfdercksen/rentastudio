...
export interface ModificationEmailInput {
  clientName: string;
  clientEmail: string;
  bookingDate: string;
  startTime: string;
...
export async function sendModificationEmail(
  ...
  try {
    ...
    const modLabel = input.modificationType === "reschedule"
      ? "Rescheduled"
      : input.modificationType === "add_ons"
      ? "Add-ons Updated"
      : input.modificationType === "duration_change"
      ? "Duration Extended"
      : "Updated";
  ...
  <div style="max-width:600px;margin:0 auto;background:#fff;border:1px solid #e8e2d6;">
    ...
    <div style="padding:40px 32px;">
      <p style="font-size:16px;color:#3a3a34;margin:0 0 32px;">
        Hi ${escapeHtml(input.clientName)}, your booking has been updated. Here are your new details:
      </p>

      <div style="background:#f5f0e8;border-radius:8px;padding:28px;margin-bottom:28px;">
        <h2 style="margin:0 0 20px;font-family:Georgia,serif;font-size:20px;font-weight:400;color:#0e0d0b;">Updated Booking Details</h2>
      ...
      <div style="background:#0e0d0b;border-radius:8px;padding:24px;margin-bottom:28px;position:relative;overflow:hidden;">
        ...
        <table style="width:100%;border-collapse:collapse;color:#faf7f2;font-size:14px;">
          ...
          <tr style="border-top:1px solid rgba(255,255,255,0.15);">
            <td style="padding:14px 0 6px;font-family:Georgia,serif;font-size:20px;color:#c8984a;">Total</td>
            <td style="padding:14px 0 6px;text-align:right;font-family:Georgia,serif;font-size:20px;color:#c8984a;">R${input.totalAmount.toFixed(2)}</td>
          </tr>
        </table>
      </div>

      <div style="text-align:center;margin-bottom:28px;">
        <a href="${escapeHtml(input.manageUrl)}" style="display:inline-block;background:#c8984a;color:#0e0d0b;text-decoration:none;padding:12px 28px;border-radius:6px;font-size:14px;font-weight:600;">
          Manage My Booking →
        </a>
      </div>
    ...
    const { error } = await resend.emails.send({
      ...
      subject: `Booking Updated — ${input.bookingDate} at ${input.startTime}`,
      html,
    });

    if (error) {
      console.error("sendModificationEmail failed:", error.message);
    ...
    console.error("sendModificationEmail failed:", message);
